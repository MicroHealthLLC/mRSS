class MeetingValidator < ActiveModel::Validator
  def validate(record)
    joined_meeting = Meeting.joins(:meeting_dates).
        ...
        where(meeting_dates: {date: record.meeting_dates.map(&:date)}).
      ...
      if record.new_record? and record.meeting_dates.any? {|md| md.date.to_date < Date.today.to_date}
        record.errors[:base]<<  'Cannot save meeting from the past'
      end

      if record.meeting_dates.blank?
        record.errors[:base]<<  'No dates selected'
      end


      if (d = record.meeting_dates.detect{|md| md.date.to_date == Date.today.to_date})
